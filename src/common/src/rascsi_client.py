...
from rascsi_constants import REMOVABLE_DEVICE_TYPES
from socket_cmds import SocketTools


class RaScsiClient:

    def __init__(self, socket_tools: SocketTools, host='localhost', port=6868, token="", locale='en'):
        self.host = host
        self.port = port
        self.socket_tools = socket_tools
        self.token = token
        self.locale = locale
    ...
    def get_server_info(self):
        ...
        data = self.socket_tools.send_pb_command(command.SerializeToString(), self.host, self.port)
